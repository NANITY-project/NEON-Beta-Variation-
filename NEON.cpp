...
static void ensure_cache_room(const engine::Config& cfg, EngineState& st, size_t seq) {
    ...
    constexpr size_t kSinkTokens = 4;   // StreamingLLM: a handful of sink tokens is enough
    size_t keep = std::min(kSinkTokens, st.cache.length);
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0]
                  << " --model <path.gguf|path.nctr>"
                     " [--prompt <text_or_json> | --interactive | --probe]"
                     " [--threads N]\n";
        return 1;
    }
        ...
        switch (fmt) {
            case ModelFormat::GGUF:
                ...
                return run_model<loader::GGUFLoader>(
                    model_path, cfg, pool, probe_only, interactive, input_payload, t_start);
