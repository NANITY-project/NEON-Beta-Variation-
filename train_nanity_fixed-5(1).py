...
def collate_fn(batch, pad_id: int = 0):
    ...
    PERF FIX: the inner "for j, is_loss in enumerate(msk)" loop iterated
    the Python interpreter once per TOKEN (up to context_length, e.g.
    4096-8192) for every example in every batch -- a real, easily avoided
    cost given this runs on every single training step. Vectorized to one
    tensor op per example instead of one Python-level op per token.
    """
    ...
    for i, (ids, msk, is_ans) in enumerate(zip(ids_list, mask_list, is_answer_list)):
        ...
        msk_t    = torch.tensor(msk, dtype=torch.bool)
        is_ans_t = torch.tensor(is_ans, dtype=torch.bool)
        input_ids[i, :n]   = ids_t
        attn_mask[i, :n]   = True
        labels[i, :n]      = torch.where(msk_t, ids_t, labels[i, :n])
        answer_mask[i, :n] = msk_t & is_ans_t
...
def train(args):
    ...
    loader = torch.utils.data.DataLoader(
        ...
        collate_fn=collate,
        drop_last=True,
        # PERF FIX: data_iter() does `while True: yield from loader` --
        # every pass through the dataset re-iterates the DataLoader, and
        # without persistent_workers=True that tears down and respawns all
        # 4 worker processes at the start of every single epoch. For a
        # dataset that's short relative to --steps (common for finetune
        # runs with many epochs), that's a real, repeated stall where the
        # GPU sits idle waiting for workers to spin back up. prefetch_factor
        # raised so 4 workers can stay further ahead of a GPU that (after
        # the OOM/logits fixes above) is doing much less per-batch CPU wait.
    ...
    for input_ids, labels, attn_mask, answer_mask in data_iter():
        ...
        if step % save_every == 0 or step == start_step + args.steps:
            if args.lora_rank > 0:
                ...
                save_checkpoint(model, optimizer, step, loss.item(), out_dir, cfg, train_meta)
        ...
        if val_loader is not None and step % args.val_every == 0 and step > start_step:
            ...
            if track_metric < best_val_loss - args.early_stop_min_delta:
                ...
                if args.lora_rank > 0:
                    ...
                    saved_path = save_checkpoint(model, optimizer, step, loss.item(), out_dir, cfg, train_meta)
