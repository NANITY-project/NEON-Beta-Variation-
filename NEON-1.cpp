...
// doesn't also carry.
static void ensure_cache_room(const engine::Config& cfg, EngineState& st, size_t seq,
                               size_t keep = 4) {
    if (st.cache.length + seq <= st.cache.capacity) return;
                           ...
                           float  top_p)
{
    ...
    // ---- Build the pinned region: seed block ("implant" stand-in for this
    // step) followed by optional dedicated neutral sink tokens.
    ...
    std::cerr << "[Idle] pinned region: " << keep << " tokens ("
              << seed_len << " seed + " << sink_tokens << " dedicated sink), "
    ...
    while (max_tokens == 0 || produced < max_tokens) {
        if (next == tok.eos_id) {
            ...
            std::cout << tok.decode_one(next) << std::flush;
            ++produced;
        }

        ensure_cache_room(cfg, st, 1, keep);   // may trigger a shift; VLOGs internally
        std::vector<int32_t> one_token{ next };
        fwd::transformer_forward(cfg, st.mw, one_token, st.cache, logits.data(), pool);
        st.cached_tokens.push_back(next);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0]
                  << " --model <path.gguf|path.nctr>"
                     ...
                     "   --idle-seed <text>        pinned seed/implant-stand-in block (required)\n"
                     "   --idle-sink-tokens <N>     dedicated neutral sink tokens appended after the seed (default 0)\n"
                     "   --idle-max-tokens <N>      stop after N generated tokens, 0 = run forever (default 0)\n"
                     "   --idle-log-every <N>       log cache stats every N tokens (default 50)\n"
    ...
    if (idle.enabled && idle.seed_text.empty()) {
        std::cerr << "[Error] --idle-loop requires --idle-seed <text>.\n";
        return 1;
    }
