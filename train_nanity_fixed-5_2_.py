...
@dataclass
class Example:
    # Packed as array.array, NOT plain Python list[int]/list[bool].
    #
    # A plain `list[int]` stores one boxed Python int object PER TOKEN --
    # ~28 bytes of object overhead each (token ids are almost never in
    # CPython's small-int cache of -5..256), plus 8 bytes for the list's
    # own pointer slot. At pretrain scale (millions of examples, billions
    # of tokens) that's 150GB+ of pure interpreter overhead sitting on top
    # of the actual data -- which is exactly what OOM-killed a 5.17M-line
    # tokenization pass here. array.array stores a real packed C buffer:
    # 'H' (2 bytes/token, unsigned short) if the tokenizer's vocab fits in
    # 16 bits, else 'I' (4 bytes/token). mask/is_answer use 'B' (1 byte
    # each, 0/1) instead of a list of Python bool objects. Net effect:
    # roughly 10-20x less RAM for the same corpus, and a much smaller/
    # faster-loading on-disk cache too, since pickling array.array stores
    # the raw bytes rather than one pickled object per element.
    #
    # Fully compatible with existing consumers: array.array supports
    # len()/indexing/iteration, so collate_fn's torch.tensor(ids) and
    # per-token loops work unchanged.
    ids:       "array.array"
    mask:      "array.array"   # True(1)/False(0) = compute loss here
    is_answer: "array.array"   # True(1)/False(0) = final-answer token (only meaningful where mask==1)
